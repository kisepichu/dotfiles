...
9. `allow`/`deny` をフック JSON に変換して出力。`ask`・タイムアウト・エラーは
   **出力なし＝人間へ**。
10. `log_decisions` に含まれる判定だけ `logs/audit.jsonl` に追記（既定で `ask` は残さない）。
...
- **無効時は1行も書かない**（以前は呼び出しごとに `stage="disabled"` を記録していた）。
- `log_decisions`（既定 `["allow","deny","answer"]`）に含まれる判定だけを記録。
  ルーチンの `ask`（ハード規則エスカレーション含む）は UI で見えるため既定で残さない。
  デバッグ時は `"ask"` を加えれば従来どおり全件記録できる。
...
| `log_decisions` | 監査ログに残す判定の配列（既定 `["allow","deny","answer"]`、`ask` は既定で残さない）。 |
...
- **監査ログ** `logs/audit.jsonl`（chezmoi 管理外、実行時生成）。無効時は1行も書かず、
  `log_decisions` に含まれる判定のみ記録（既定で `ask` は残さない）。各行に
