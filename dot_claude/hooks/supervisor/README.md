...
- **`cd` を追跡して評価 cwd を更新**: 複合の途中で `cd` するとそれ以降のセグメントは
  新しい cwd で判定する（scratch 包含判定や下記の相対パス解決を実際の実行と一致させる
  ため）。例えば `/tmp/x` から `cd / && rm -rf etc` を評価すると、`rm` は `/etc`
  （scratch 外）と解決され自動許可されない。`cd` 先を厳密に解決できない場合（`cd -`/
  OLDPWD・オプション付き・複数引数・`~`・cwd 不明の相対パス）は複合許可を**中止**。
  - **`cd` 先は実在ディレクトリ必須**: `cd` は成功して初めて cwd を移すため、`cd` 先が
    実在ディレクトリでなければ中止する。これにより `cd /tmp/x/NOPE; rm -rf build` の
    ように `cd` が失敗して `rm` が元の cwd で走るケースを scratch と誤認しない。
  - **サブシェルを含む複合は中止**: パイプ `|`・バックグラウンド `&` はサブシェルを生み、
    その中の `cd` は親シェルの cwd を変えない。`|`/`&` を含む複合で実際に cwd を移す
    `cd` があれば、cwd 追跡が不健全になるため中止する。
- **相対パスを cwd で解決してハード規則に通す**: 各セグメントのハード規則判定では、
  相対パスのオペランドを追跡中の cwd で絶対パスに解決して haystack に加える。これにより
  `cd ~/.kube && cat config` のように `cd` で機微ディレクトリへ移動してから素のファイル名
  で読むパターン（セグメント単体では `cat config` で `.kube/config` が見えない）も
  パスベースのハード規則で捕捉して人間へ回す。
