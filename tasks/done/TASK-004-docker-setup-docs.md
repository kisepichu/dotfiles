...
- 2026-05-15: test WSL distro で Docker Engine install script を実行。systemd 有効化後、通常ユーザーで `docker run --rm hello-world` と `docker compose version` が通過。
- 2026-05-15: PR review 対応で README の WSL/systemd 手順と Docker verify command を修正。script は conflict package 検出削除と docker group opt-in を追加し、test WSL distro で再実行確認。
